...
{  
    ...
    public static String sendPostRequest(String reqURL, String reqData, String application, String encodeCharset) {
        String reseContent = "通信失败";  
        HttpClient httpClient = new DefaultHttpClient();  
        httpClient.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, 10000);  
        httpClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, 20000);  
        ...
        //由于下面使用的是new StringEntity(....),所以默认发出去的请求报文头中CONTENT_TYPE值为text/plain; charset=ISO-8859-1  
        //这就有可能会导致服务端接收不到POST过去的参数,比如运行在Tomcat6.0.36中的Servlet,所以我们手工指定CONTENT_TYPE头消息  
        httpPost.setHeader(HTTP.CONTENT_TYPE, "application/" + application + "; charset=" + encodeCharset);
        try {
            httpPost.setEntity(new StringEntity(reqData == null ? "" : reqData, encodeCharset));
            HttpResponse response = httpClient.execute(httpPost);  
            HttpEntity entity = response.getEntity();  
    ...
    public static int sendPostFiles(String URLString,InputStream in) {
        try {
            ...
            InputStreamRequestEntity fr = new InputStreamRequestEntity(in);
            ...
            try {
                ...
                if (method.getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    BufferedReader reader = new BufferedReader(
                            new InputStreamReader(method.getResponseBodyAsStream(), "GBK"));
                    String line;
            ...
            logger.info("执行HTTP Post请求" +response.toString());
            return 1;
        } catch (Exception e) {
            String requestURI = WebContext.getRequest().getRequestURI();
